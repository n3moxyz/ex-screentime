...
Open `http://127.0.0.1:5173/`, choose a replay fixture, and start replay mode. The default `Strong Harness Replay` is the main judging demo path.

## What Works

- Replay Fixture Mode is the default and is clearly labeled.
- Deterministic strong, medium, and weak fixtures drive the same UI.
...
- Five-lens panel presets are selectable for Overall, Impact, Harness / Skills, Technical Execution, and Demo Readiness tracks.
- Custom panel selection supports 3 to 5 evaluator lenses.
- Track presets foreground the relevant rubric dimensions in the scorecard and evidence inspector.
- Local Path Mode performs read-only static inspection of safe files and emits the same structured event stream without running repo commands.
...
Visual smoke opens the app in headless system Chrome at desktop and mobile widths, checks Local Path Mode against this repo, fixture switching, panel preset switching, Compare Fixtures, starts replay, waits for completion, opens the Rubric and Judge Report tabs, verifies the final score text, triggers JSON export, checks for horizontal overflow, and writes ignored screenshots to `ledger/visual-smoke-*.png`.
...
| Metric | Status | Notes |
| --- | --- | --- |
| Demo Success | 8/10 | Phase 0 flow is end-to-end and under 90 seconds; calibration fixtures, panel presets, and static local inspection are selectable. |
